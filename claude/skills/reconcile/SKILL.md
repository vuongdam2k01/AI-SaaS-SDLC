...
This session must run at the root of the dedicated documentation repository — never inside an application codebase. Set `ENGINE` to `node "${CLAUDE_PLUGIN_ROOT}/bin/ai-saas-sdlc"`. Use Claude Code file/Git tools to inspect the cited mismatch and only configured implementation roots; execute only engine-declared verification commands.
