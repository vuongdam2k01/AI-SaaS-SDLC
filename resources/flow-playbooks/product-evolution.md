...
The same doctrine covers implementation mappings themselves. A feature whose declaring artifacts carry no mapping, or a specified test level no mapped test implements, is a standing warning (`IMPLEMENTATION_MAPPING_MISSING`, `IMPLEMENTATION_LEVEL_UNPROVEN`), never a baseline error: a documentation-first evolution in a wired repository closes honestly, and the warning ledger is the durable record of the docs-to-code distance. Read `generated/implementation-coverage.md` for the per-feature state and `generated/implementation-plan/<FTR-ID>.md` for the joined work packet before editing mapped code.
