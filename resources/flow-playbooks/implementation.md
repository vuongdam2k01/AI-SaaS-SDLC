...
4. The `Engineering profile` section of `ARCHITECTURE-OVERVIEW` — language, framework, package manager, repository layout, test frameworks per level, migration tool, and the solution-class rows (HTTP serving and routing, validation, forms, styling plumbing, animation, state, data fetching, jobs/scheduling, auth mechanics). Empty or missing profile on a first implementation: fill it in this flow (it is an owned section of an active foundation, a normal edit) from what the codebase itself evidences. Where the codebase evidences nothing — a fresh scaffold, an untouched solution class — the substrate choices are the **author's material decisions, not the implementer's defaults**: present the candidates per solution class through the host's interactive ask facility (options, consequences, a marked recommendation), honor a stack the invocation already states, and in a non-interactive run record the choice as a `QST-*` with `Blocked on: owner-decision` and take the ladder's lowest holding rung as the provisional, profile-recorded answer. Scaffolding a stack silently is the defect that costs an author their codebase.
...
1. Edit only mapped paths and paths the segment is adding, inside configured roots. Preserve the Engineering profile's conventions; where the profile is silent and the codebase shows a convention, follow the codebase and record the convention in the profile. Where the profile is silent **and** the codebase shows nothing — the first time a segment touches a commodity solution class — the scouting protocol's solution ladder decides: reuse → stdlib → native platform → installed dependency → a recorded new-dependency-or-custom decision. Inventing silently is not a rung.
