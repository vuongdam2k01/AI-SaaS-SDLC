...
- **Match capability to judgment, cost to mechanics.** Discovery, suite-running and mechanical sweeps take the cheap tier; review, design judgment and diagnosis take the strong tier. Never budget-route judgment. A delegate's declared tier is a **floor**, not a ceiling: where the host allows a per-spawn model choice, a session running a stronger model spawns the delegate at the session's model — overriding a judgment role downward is budget-routing and forbidden, whatever the session's own tier happens to be.
