...
6. **Solution ladder.** For every component the segment will write that belongs to a commodity solution class — HTTP serving and routing, request/response validation, form handling, styling and design-token plumbing, animation, state management, data fetching and caching, background jobs and scheduling, auth/session mechanics, date/time, file upload and parsing, logging — first read the Engineering profile row that governs it (a row naming a choice is binding), then climb and **stop at the first rung that holds**, stating the rung taken in the scout output:
   ...
   The ladder runs *after* the scout understands the problem, never instead of it — lazy about the solution, never about the reading. A deliberate simplification that cuts a real corner with a known ceiling (a global lock, an O(n²) scan, an in-process queue that does not redeliver) is marked at the code site with a `ceiling:` comment in the fixed, greppable form — `// ceiling: <the limit>; upgrade: <the trigger to revisit>` — and named in the closing report: the profile row records the decision, the marker makes it findable where it bites. A marker naming no upgrade trigger is the kind that silently rots; review flags it, and Inspect State harvests the whole set so a deferral cannot quietly become permanent.
