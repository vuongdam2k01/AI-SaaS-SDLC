...
| `verify` | `--unit`, `--integration`, `--system`, `--all`, required `--execute`, `--json` | Runs exact commands from `sdlc.config.yaml` only during Evolution or Reconciliation. With no level flag, all three levels are selected. Every attempt reserves a new `EXEC-*` and renders `RESULT-EXEC-*`; failures remain in history and return non-zero. Machine-local budgets apply (timeout kills and records `timed_out`; oversized output is cut and records `output_truncated`; a command that never ran records `spawn_error` as an environment failure). A command with a declared `report` gets its junit/TAP report parsed into per-case results joined to specification mapping rows. A completed run resynchronizes generated projections, so record-dependent views such as `generated/platform-coverage.md` and `generated/implementation-coverage.md` stay current mid-flow. |
