...
- **Validation errors: 0.** Warnings are informative and never block a baseline: `RULE_UNVERIFIED` (a business rule no spec claims yet), `SPEC_OVERSIZED` (a spec to split), `QUESTION_STALE` (an old open question), `CASE_REFERENCE_BROKEN`, `AREA_UNREGISTERED`, the three `*_AUTHORITY_UNDECLARED` contract warnings, the platform and research families — the ripple family (`IMPACT_UNCLASSIFIED` for an artifact the change put in question and nobody decided about, `SPEC_EXECUTION_UNATTRIBUTED` for a selected spec no report reaches, `CLAIM_WITHOUT_DEPENDENCY` for a claim the closure cannot see) — and, once implementation sources are configured, seven more: `IMPLEMENTATION_MAPPING_MISSING`, `IMPLEMENTATION_LEVEL_UNPROVEN`, `IMPLEMENTATION_DRIFT`, `DOCUMENTATION_DRIFT`, `IMPLEMENTATION_SYMBOL_MISSING`, `IMPLEMENTATION_MAPPING_ROW_IGNORED` and `IMPLEMENTATION_MAPPING_PATH_MISSING`. All thirty-two are listed with their remedies in [Inspect state and check results](inspect-and-check-results.md), including the five closure warnings that name a declared access rule, invariant, error code, UX rule or screen behavior nothing verifies, `DESIGN_TOKENS_UNCOMMITTED` for live screens rendering with no committed visual system and no deferring question, and `SYSTEM_DOCUMENT_INCOMPLETE` for a `00-system` document that has fallen behind its contract. They are work owed, reported precisely so it stays visible.
