...
3. **Real public-web research** — the flow probes for optional self-hosted instruments first (`research probe`); with SearXNG/Firecrawl/Camofox configured it retrieves through the engine, leaving `QRY-*`/`RET-*` provenance records, and otherwise uses live `WebSearch` and `WebFetch` to inspect actual pages: the candidate segment, the problem and its workarounds, the five tiers of alternatives (direct, indirect, DIY, general-purpose, status quo), pricing, market/timing, and feasibility risks. Every material source becomes an attributable `EVD-*` entry in the evidence ledger, citing its retrieval record when the engine fetched the page (see [research tools](../research-tools.md)). *If real web tools are unavailable, the flow stops and says so — it never substitutes guesses for research.*
