...
4. **Research only the delta** — engine retrieval when self-hosted instruments are configured (`research probe` reports what is reachable; `research search`/`fetch` retrieve, `research map` enumerates a competitor's pricing, security and documentation URLs, `research crawl` captures a bounded subtree, and `research diff` gives a deterministic answer to "has this page changed since we recorded it?"), and otherwise live `WebSearch`/`WebFetch` on the actual pages. New observations become new `EVD-*` entries, each citing its `RET-*` record when the engine fetched the page. **History is never rewritten**: a changed time-sensitive fact keeps the old entry and adds a successor with its current applicability. *If real web tools are unavailable, the flow stops honestly rather than claiming reassessment.*
