...
The engine also regenerates cross-cutting views into `generated/` after every flow — traceability, the artifact graph and index, rule and acceptance coverage, `foundation-coverage.md` and `screen-coverage.md` once foundation rules and screens exist, feature and implementation maps, stale artifacts, issue and decision indexes, `platform-coverage.md` once the product ships on a declared platform, and `implementation-coverage.md` plus one `implementation-plan/<FTR-ID>.md` work packet per feature once implementation sources are configured. Read them; never edit them.
