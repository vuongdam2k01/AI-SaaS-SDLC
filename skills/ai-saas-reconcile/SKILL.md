...
Resolve `PLUGIN_ROOT` as two parent directories above this installed `SKILL.md`; invoke the engine as `node "<PLUGIN_ROOT>/bin/ai-saas-sdlc"`. Inspect cited files, Git diffs/results and only configured implementation roots. Execute only configured verification through the engine.
