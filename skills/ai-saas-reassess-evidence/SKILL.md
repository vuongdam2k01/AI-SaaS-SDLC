...
Resolve `PLUGIN_ROOT` as two parent directories above this installed `SKILL.md`; invoke the engine as `node "<PLUGIN_ROOT>/bin/ai-saas-sdlc"`. Use Codex's actual Internet search and page-open/fetch tools and inspect every cited source. Stop honestly if they are unavailable.
