...
The seven cases grade installed-skill behavior from three captured surfaces: the initialized documentation repository, the host tool trace, and the final response. A fluent response cannot compensate for missing files, uninspected sources, direct RESULT writes, or prohibited procedure. The implementation case additionally requires a wired sibling codebase fixture (a minimal `node:test` scaffold with one deliberately red suite), so its capture includes real failed-then-repaired execution records.
