...
function parseFrontmatter(content, file) {
  ...
  return { data: parsed, body: normalized2.slice(end + 5) };
}
...
function projectPaths(root2) {
  return {
    root: root2,
    config: path.join(root2, CONFIG_FILE),
    current: path.join(root2, INTERNAL_DIR, "state", "current.json"),
...
function isWithin(root2, candidate) {
  const relative = path.relative(path.resolve(root2), path.resolve(candidate));
  return relative === "" || !relative.startsWith("..") && !path.isAbsolute(relative);
}
async function assertSafeManagedPath(root2, candidate) {
  const absoluteRoot = path.resolve(root2);
...
async function loadConfig(root2) {
  ...
  try {
    ...
    parsed = import_yaml2.default.parse(await readFile(file, "utf8"));
  } catch (error) {
...
var require_sync = __commonJS({
  "node_modules/@nodelib/fs.stat/out/providers/sync.js"(exports) {
    ...
    function read(path3, settings) {
      const lstat2 = settings.fs.lstatSync(path3);
      if (!lstat2.isSymbolicLink() || !settings.followSymbolicLink) {
        return lstat2;
      }
      try {
...
var root = typeof input.cwd === "string" ? input.cwd : process.cwd();
var mutatingDirectTool = /* @__PURE__ */ new Set(["Write", "Edit", "MultiEdit", "NotebookEdit", "apply_patch"]);
...
async function protectedOriginal(value) {
  ...
  try {
    const content = await readFile3(path2.isAbsolute(value) ? value : path2.resolve(root, value), "utf8");
...
async function protectedAdr(value) {
  if (!/(?:^|[/\\])05-control[/\\]decisions[/\\]ADR-[^/\\]+\.md$/i.test(value)) return null;
  try {
    const meta = toArtifactMeta(parseFrontmatter(await readFile3(path2.resolve(root, value), "utf8"), value).data);
    return meta.adr_status === "accepted" ? "Accepted ADR bodies are immutable. Create a successor ADR with supersedes." : null;
  } catch {
    return null;
  }
}
async function protectedTerminalArtifact(value) {
  if (!/(?:^|[/\\])0[1-5]-[^/\\]+[/\\].+\.md$/i.test(value)) return null;
  try {
    const file = path2.isAbsolute(value) ? value : path2.resolve(root, value);
    const meta = toArtifactMeta(parseFrontmatter(await readFile3(file, "utf8"), value).data);
...
var candidatePaths = [];
for (const key of ["file_path", "path", "notebook_path"]) if (typeof toolInput[key] === "string") candidatePaths.push(toolInput[key]);
...
var implementationFlow = false;
if (await pathExists(projectPaths(root).current)) {
  try {
    const config = await loadConfig(root);
    implementationRoots = config.implementation_sources.map((source) => path2.resolve(root, source.path));
    implementationMarkers = config.implementation_sources.flatMap((source) => [source.path, path2.resolve(root, source.path)]).map((value) => value.replaceAll("\\", "/").toLowerCase());
    const flow = await loadActiveFlow(root);
...
if (!reason && !implementationFlow && implementationRoots.length > 0) {
  for (const candidate of candidatePaths) {
    const absolute = path2.isAbsolute(candidate) ? path2.resolve(candidate) : path2.resolve(root, candidate);
